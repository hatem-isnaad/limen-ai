...
class ApprovalController
{
    ...
    protected function resolve(Request $request, string $approvalId, bool $approve): JsonResponse
    {
        ...
        if ($approve && $request->filled('tool_input') && is_array($request->input('tool_input'))) {
            $checkpoint = $this->checkpoints->load($runId);

            if (is_array($checkpoint)) {
                $state = $checkpoint['state'] ?? $checkpoint;

                if (isset($state['pending_approval']) && is_array($state['pending_approval'])) {
                    $state['pending_approval']['input'] = $request->input('tool_input');
                    $checkpoint['state'] = $state;
                    $this->checkpoints->save($runId, (int) ($checkpoint['step'] ?? 0), $state);
                }
            }
        }
